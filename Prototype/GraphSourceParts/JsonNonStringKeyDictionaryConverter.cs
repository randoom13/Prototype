...
{
    sealed class JsonNonStringKeyDictionaryConverter<TKey, TValue> : JsonConverter<IDictionary<TKey, TValue>> where TKey : notnull, new()
    {
    ...
    sealed class JsonNonStringKeyDictionaryConverterFactory : JsonConverterFactory
    {
        ...
        {
            if (!typeToConvert.IsGenericType || typeToConvert.GenericTypeArguments[0] == typeof(string)) return false;

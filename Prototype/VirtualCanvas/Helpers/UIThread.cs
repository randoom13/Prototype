...
namespace Prototype.Main.VirtualCanvas.Helpers
{
    ...
    internal static class UIThread
    {
            ...
            public override bool WaitOne(int millisecondsTimeout)
            {
                if (IsUIThread(Thread.CurrentThread))
                {
                    ...
                    while (!task.IsCompleted && !task.IsCancelled)
                    {
                        ...
                        while (RunTask(task) && !task.IsCancelled)
                        {
        ...
        public static void Initialize()
        {
            ...
            Debug.Assert(!initialized || TestEnvironment || UIThread.uiThread == uiThread, "There can only be one UI thread!");
            if (!initialized)
            {
                uiTasks = new List<TaskData>();
                UIThread.uiThread = uiThread;
                initialized = true;
                timer = new DispatcherTimer(TimeSpan.FromMilliseconds(1), DispatcherPriority.Normal, new EventHandler(OnTick), Dispatcher.CurrentDispatcher);
                timer.IsEnabled = false;
                uiDispatcher = Dispatcher.FromThread(uiThread);
            }
        }
